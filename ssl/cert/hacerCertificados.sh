...
DAYS=$((3*365))
LEVELS=4
DN="/C=CL/ST=RM/L=Santiago/O=Example Corp./OU=PKI"
CHAIN=chain.crt
...
for i in `seq 1 $LEVELS`; do
    ...
    #generate key pair
    openssl ecparam -genkey -name secp384r1 | openssl ec -out ca$i-key.pem
...
#generate key pair
openssl ecparam -genkey -name secp384r1 | openssl ec -out client.key
...
#generate key pair
openssl ecparam -genkey -name secp384r1 | openssl ec -out agent1-key.pem
...
#generate key pair
openssl ecparam -genkey -name secp384r1 | openssl ec -out agent2-key.pem
